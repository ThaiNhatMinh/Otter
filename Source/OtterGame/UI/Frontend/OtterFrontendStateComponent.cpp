...
#include "OtterFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterFrontendStateComponent)
...
UOtterFrontendStateComponent::UOtterFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UOtterFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UOtterFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UOtterFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UOtterFrontendStateComponent::OnExperienceLoaded(const UOtterExperienceDefinition* Experience)
{
...
void UOtterFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UOtterFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UOtterFrontendStateComponent::OnUserInitialized);
...
void UOtterFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UOtterFrontendStateComponent::OnUserInitialized);
...
void UOtterFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UOtterFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
