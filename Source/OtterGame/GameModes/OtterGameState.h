...
#include "OtterGameState.generated.h"
...
class UOtterExperienceManagerComponent;
class UObject;
struct FFrame;
 ...
 */
UCLASS(Config = Game)
class OTTERGAME_API AOtterGameState : public AModularGameStateBase, public IAbilitySystemInterface
{
	GENERATED_BODY()

public:

	AOtterGameState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	UFUNCTION(BlueprintCallable, Category = "Otter|GameState")
	...
	UFUNCTION(NetMulticast, Unreliable, BlueprintCallable, Category = "Otter|GameState")
	void MulticastMessageToClients(const FOtterVerbMessage Message);

	// Send a message that all clients will be guaranteed to get
	// (use only for client notifications that cannot handle being lost)
	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "Otter|GameState")
	void MulticastReliableMessageToClients(const FOtterVerbMessage Message);
...
private:
	...
	UPROPERTY()
	TObjectPtr<UOtterExperienceManagerComponent> ExperienceManagerComponent;

	// The ability system component subobject for game-wide things (primarily gameplay cues)
	UPROPERTY(VisibleAnywhere, Category = "Otter|GameState")
