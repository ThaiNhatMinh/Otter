...
#include "OtterReplaySubsystem.generated.h"
...
class OTTERGAME_API UOtterReplayListEntry : public UObject
{
	GENERATED_BODY()
...
UCLASS(BlueprintType)
class OTTERGAME_API UOtterReplayList : public UObject
{
	GENERATED_BODY()

public:
	UPROPERTY(BlueprintReadWrite, Category=Replays)
	TArray<TObjectPtr<UOtterReplayListEntry>> Results;
};

/** Subsystem to handle recording/loading replays */
UCLASS()
class OTTERGAME_API UOtterReplaySubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

public:
	UOtterReplaySubsystem();
	...
	void PlayReplay(UOtterReplayListEntry* Replay);
