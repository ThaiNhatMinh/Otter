...
#include "Cosmetics/OtterCosmeticAnimationTypes.h"
#include "OtterCharacterPartTypes.h"
#include "Net/Serialization/FastArraySerializer.h"

#include "OtterPawnComponent_CharacterParts.generated.h"

class UOtterPawnComponent_CharacterParts;
namespace EEndPlayReason { enum Type : int; }
struct FGameplayTag;
struct FOtterCharacterPartList;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOtterSpawnedCharacterPartsChanged, UOtterPawnComponent_CharacterParts*, ComponentWithChangedParts);

//////////////////////////////////////////////////////////////////////

// A single applied character part
USTRUCT()
struct FOtterAppliedCharacterPartEntry : public FFastArraySerializerItem
{
	GENERATED_BODY()

	FOtterAppliedCharacterPartEntry()
	{}

	FString GetDebugString() const;

private:
	friend FOtterCharacterPartList;
	friend UOtterPawnComponent_CharacterParts;
...
struct FOtterCharacterPartList : public FFastArraySerializer
{
	GENERATED_BODY()

	FOtterCharacterPartList()
...
public:
	...
	{
		return FFastArraySerializer::FastArrayDeltaSerialize<FOtterAppliedCharacterPartEntry, FOtterCharacterPartList>(Entries, DeltaParms, *this);
	}

	FOtterCharacterPartHandle AddEntry(FOtterCharacterPart NewPart);
	void RemoveEntry(FOtterCharacterPartHandle Handle);
	void ClearAllEntries(bool bBroadcastChangeDelegate);

	FGameplayTagContainer CollectCombinedTags() const;

	void SetOwnerComponent(UOtterPawnComponent_CharacterParts* InOwnerComponent)
	{
...
private:
	...
	bool SpawnActorForEntry(FOtterAppliedCharacterPartEntry& Entry);
	bool DestroyActorForEntry(FOtterAppliedCharacterPartEntry& Entry);

private:
	// Replicated list of equipment entries
	UPROPERTY()
	TArray<FOtterAppliedCharacterPartEntry> Entries;

	// The component that contains this list
	UPROPERTY(NotReplicated)
	TObjectPtr<UOtterPawnComponent_CharacterParts> OwnerComponent;
...
template<>
struct TStructOpsTypeTraits<FOtterCharacterPartList> : public TStructOpsTypeTraitsBase2<FOtterCharacterPartList>
{
...
class UOtterPawnComponent_CharacterParts : public UPawnComponent
{
	GENERATED_BODY()

public:
	UOtterPawnComponent_CharacterParts(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	FOtterCharacterPartHandle AddCharacterPart(const FOtterCharacterPart& NewPart);
	...
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category=Cosmetics)
	void RemoveCharacterPart(FOtterCharacterPartHandle Handle);
...
public:
	...
	FOtterSpawnedCharacterPartsChanged OnCharacterPartsChanged;

private:
	// List of character parts
	UPROPERTY(Replicated, Transient)
	FOtterCharacterPartList CharacterPartList;

	// Rules for how to pick a body style mesh for animation to play on, based on character part cosmetics tags
	UPROPERTY(EditAnywhere, Category=Cosmetics)
	FOtterAnimBodyStyleSelectionSet BodyMeshes;
};
