...
#include "OtterCameraMode_ThirdPerson.h"
#include "Camera/OtterCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/OtterPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "OtterCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterCameraMode_ThirdPerson)

namespace OtterCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UOtterCameraMode_ThirdPerson::UOtterCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FOtterPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FOtterPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOtterPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOtterPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOtterPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOtterPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FOtterPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UOtterCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UOtterCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UOtterCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UOtterCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IOtterCameraAssistInterface* TargetControllerAssist = Cast<IOtterCameraAssistInterface>(TargetController);

	IOtterCameraAssistInterface* TargetActorAssist = Cast<IOtterCameraAssistInterface>(TargetActor);
	...
	IOtterCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IOtterCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IOtterCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IOtterCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UOtterCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IOtterCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FOtterPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(OtterCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UOtterCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UOtterCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
