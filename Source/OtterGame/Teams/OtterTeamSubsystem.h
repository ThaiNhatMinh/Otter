...
#include "OtterTeamSubsystem.generated.h"

class AActor;
class AOtterPlayerState;
class AOtterTeamInfoBase;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnOtterTeamDisplayAssetChangedDelegate, const UOtterTeamDisplayAsset*, DisplayAsset);

USTRUCT()
struct FOtterTeamTrackingInfo
{
	GENERATED_BODY()

public:
	UPROPERTY()
	TObjectPtr<AOtterTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<AOtterTeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<UOtterTeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnOtterTeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(AOtterTeamInfoBase* Info);
	void RemoveTeamInfo(AOtterTeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class EOtterTeamComparison : uint8
{
...
UCLASS()
class OTTERGAME_API UOtterTeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	bool RegisterTeamInfo(AOtterTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	bool UnregisterTeamInfo(AOtterTeamInfoBase* TeamInfo);
	...
	const AOtterPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	EOtterTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	EOtterTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UOtterTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UOtterTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	void NotifyTeamDisplayAssetModified(UOtterTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnOtterTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FOtterTeamTrackingInfo> TeamMap;
