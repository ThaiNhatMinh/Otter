...
#include "OtterAudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct FOtterAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 * Otter Audio Settings.
 */
UCLASS()
class OTTERGAME_API UOtterAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the Otter Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the Otter Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the Otter Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the Otter Audio Settings and linked to the UI and game settings in OtterSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the Otter Audio Settings and linked to the UI and game settings in OtterSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the Otter Audio Settings and linked to the UI and game settings in OtterSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the Otter Audio Settings and linked to the UI and game settings in OtterSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> DialogueControlBus = nullptr;

	// VoiceChat Sound Control Bus retrieved from the Otter Audio Settings and linked to the UI and game settings in OtterSettingsLocal
	UPROPERTY(Transient)
	...
	UPROPERTY(Transient)
	TArray<FOtterAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FOtterAudioSubmixEffectsChain> LDRSubmixEffectChain;
