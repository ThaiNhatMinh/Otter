...
#include "OtterAudioSettings.generated.h"
...
USTRUCT()
struct OTTERGAME_API FOtterSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "OtterAudioSettings"))
class OTTERGAME_API UOtterAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FOtterSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FOtterSubmixEffectChainMap> LDRAudioSubmixEffectChain;
