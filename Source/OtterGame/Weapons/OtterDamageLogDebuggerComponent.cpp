...
#include "OtterDamageLogDebuggerComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterDamageLogDebuggerComponent)

UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Otter_Damage_Message);

UOtterDamageLogDebuggerComponent::UOtterDamageLogDebuggerComponent(const FObjectInitializer& ObjectInitializer)
...
void UOtterDamageLogDebuggerComponent::BeginPlay()
{
	Super::BeginPlay();

	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
	ListenerHandle = MessageSubsystem.RegisterListener(TAG_Otter_Damage_Message, this, &ThisClass::OnDamageMessage);
}

void UOtterDamageLogDebuggerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
...
void UOtterDamageLogDebuggerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if ((TimeSinceDamage >= SecondsBetweenDamageBeforeLogging) && (DamageLog.Num() > 0))
	{
		...
		UE_LOG(LogOtter, Warning, TEXT("%d impacts in %d distinct frames over %.2f seconds did %.2f damage"),
			NumImpacts, NumFrames, TotalInterval, TotalDamage);
		if (TotalInterval > 0.0)
		{
			UE_LOG(LogOtter, Warning, TEXT("Interval ranged from %.1f ms to %.1f ms (avg %.1f ms)"),
				MinInterval * 1000.0, MaxInterval * 1000.0, (MaxInterval + MinInterval) / 2.0 * 1000.0);
			UE_LOG(LogOtter, Warning, TEXT("DPS %.2f"), TotalDamage / TotalInterval);
		}
		UE_LOG(LogOtter, Warning, TEXT("\n"));
	}
}

void UOtterDamageLogDebuggerComponent::OnDamageMessage(FGameplayTag Channel, const FOtterVerbMessage& Payload)
{
