...
#include "OtterWeaponStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

UOtterWeaponStateComponent::UOtterWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UOtterWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (APawn* Pawn = GetPawn<APawn>())
	{
		...
		{
			if (UOtterRangedWeaponInstance* CurrentWeapon = Cast<UOtterRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(UOtterRangedWeaponInstance::StaticClass())))
			{
...
bool UOtterWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
...
bool UOtterWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UOtterWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FOtterServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FOtterScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void UOtterWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FOtterServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FOtterScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void UOtterWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UOtterWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UOtterWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
