...
#include "OtterWeaponStateComponent.generated.h"
...
struct FOtterScreenSpaceHitLocation
{
...
struct FOtterServerSideHitMarkerBatch
{
	FOtterServerSideHitMarkerBatch() { }

	FOtterServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FOtterScreenSpaceHitLocation> Markers;
...
UCLASS()
class UOtterWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UOtterWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FOtterScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FOtterServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
