...
#include "OtterGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EOtterEditorLoadMode
{
...
namespace OtterGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Otter.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via OtterGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UOtterGameplayCueManager::DumpGameplayCues));

	static EOtterEditorLoadMode LoadMode = EOtterEditorLoadMode::LoadUpfront;
}
...
UOtterGameplayCueManager::UOtterGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UOtterGameplayCueManager* UOtterGameplayCueManager::Get()
{
	return Cast<UOtterGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UOtterGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UOtterGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogOtter, Warning, TEXT("UOtterGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UOtterGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (OtterGameplayCueManagerCvars::LoadMode)
	{
	case EOtterEditorLoadMode::LoadUpfront:
		return true;
	case EOtterEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EOtterEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UOtterGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UOtterGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UOtterGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UOtterGameplayCueManager* GCM = Cast<UOtterGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogOtter, Error, TEXT("DumpGameplayCues failed. No UOtterGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogOtter, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogOtter, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogOtter, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogOtter, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogOtter, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogOtter, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogOtter, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogOtter, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogOtter, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogOtter, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogOtter, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogOtter, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UOtterGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UOtterGameplayCueManager* StrongThis = Get())
					{
...
void UOtterGameplayCueManager::HandlePostGarbageCollect()
{
...
void UOtterGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogOtter, Warning, TEXT("UOtterGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UOtterGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (OtterGameplayCueManagerCvars::LoadMode)
	{
	case EOtterEditorLoadMode::LoadUpfront:
...
void UOtterGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UOtterGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UOtterGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UOtterGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UOtterGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UOtterGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
