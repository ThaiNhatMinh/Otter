...
#include "OtterPlayerStart.generated.h"

class AController;
class UObject;

enum class EOtterPlayerStartLocationOccupancy
{
	Empty,
	Partial,
	Full
 ...
 */
UCLASS(Config = Game)
class OTTERGAME_API AOtterPlayerStart : public APlayerStart
{
	GENERATED_BODY()

public:
	AOtterPlayerStart(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	const FGameplayTagContainer& GetGameplayTags() { return StartPointTags; }

	EOtterPlayerStartLocationOccupancy GetLocationOccupancy(AController* const ControllerPawnToFit) const;
