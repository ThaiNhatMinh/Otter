...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterContextEffectsLibrary)


void UOtterContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UOtterContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UOtterContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UOtterContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FOtterContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UOtterActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FOtterContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UOtterActiveContextEffects* NewActiveContextEffects = NewObject<UOtterActiveContextEffects>(this);
	...
	this->OtterContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UOtterContextEffectsLibrary::OtterContextEffectLibraryLoadingComplete(
	TArray<UOtterActiveContextEffects*> OtterActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(OtterActiveContextEffects);
}
