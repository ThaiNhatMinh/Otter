...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterGameInstance)

namespace Otter
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarOtterTestEncryption(
		TEXT("Otter.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarOtterUseDTLSEncryption(
		TEXT("Otter.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using Otter.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarOtterTestDTLSFingerprint(
		TEXT("Otter.TestDTLSFingerprint"),
...
UOtterGameInstance::UOtterGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UOtterGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(OtterGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(OtterGameplayTags::InitState_DataAvailable, false, OtterGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(OtterGameplayTags::InitState_DataInitialized, false, OtterGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(OtterGameplayTags::InitState_GameplayReady, false, OtterGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UOtterGameInstance::OnPreClientTravelToSession);
	}
}

void UOtterGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
AOtterPlayerController* UOtterGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AOtterPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UOtterGameInstance::CanJoinRequestedSession() const
{
...
void UOtterGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UOtterLocalPlayer* LocalPlayer = Cast<UOtterLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UOtterGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Otter::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Otter::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("OtterTest.pem");
...
void UOtterGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UOtterGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (Otter::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (Otter::bUseDTLSEncryption)
		{
