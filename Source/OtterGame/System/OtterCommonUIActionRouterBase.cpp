...
#include "System/OtterCommonUIActionRouterBase.h"
#include "Widgets/SViewport.h"
#include "OtterLogChannels.h"

void UOtterCommonUIActionRouterBase::Initialize(FSubsystemCollectionBase& Collection)
{
    Super::Initialize(Collection);
}

void UOtterCommonUIActionRouterBase::ApplyUIInputConfig(const FUIInputConfig & InNewConfig, bool bForceRefresh)
{
	...
	if (!GameViewportClient || !PC)
	{
		...
		//UE_LOG(LogOtter, Warning, TEXT("Missing GameViewportClient (%s) or PC (%s)"), BOOL2TEXT(!GameViewportClient), BOOL2TEXT(!PC));
		return;
	}
	...
	if (!ViewportWidget)
	{
		...
		UE_LOG(LogOtter, Error, TEXT("Failed to commit change! ViewportWidget is null."));
		return;
	}
	...
	//UE_LOG(LogOtter, Display, TEXT("UIInputConfig being changed. bForceRefresh: %s"), BOOL2TEXT(bForceRefresh));
	//UE_LOG(LogOtter, Display, TEXT("\tInputMode: Previous (%s), New (%s)"),		ActiveInputConfig.IsSet() ? LexToString(ActiveInputConfig->GetInputMode()) : TEXT("None"), LexToString(NewConfig.GetInputMode()));

#ifdef WITH_XIM_INPUT_DEBUG
	...
	{
		UE_LOG(LogOtter, Log, TEXT("\t\t%s"), *DebugString);
	}
#endif
	...
	UE_LOG(LogOtter, Verbose, TEXT("Broadcast Event: OnActiveInputModeChanged"));
