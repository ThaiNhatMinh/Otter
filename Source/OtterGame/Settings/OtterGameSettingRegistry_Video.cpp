...
#include "CustomSettings/OtterSettingAction_SafeZoneEditor.h"
#include "CustomSettings/OtterSettingValueDiscrete_MobileFPSType.h"
#include "CustomSettings/OtterSettingValueDiscrete_OverallQuality.h"
#include "CustomSettings/OtterSettingValueDiscrete_Resolution.h"
...
{
public:
	FGameSettingEditCondition_FramePacingMode(EOtterFramePacingMode InDesiredMode, EFramePacingEditCondition InMatchMode = EFramePacingEditCondition::EnableIf)
	...
	{
		const EOtterFramePacingMode ActualMode = UOtterPlatformSpecificRenderingSettings::Get()->FramePacingMode;
...
private:
	EOtterFramePacingMode DesiredMode;
...
public:
	...
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		if (!UOtterPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings)
		{
	...
	virtual void SettingChanged(const ULocalPlayer* LocalPlayer, UGameSetting* Setting, EGameSettingChangeReason Reason) const override
	{
		...
		const UOtterLocalPlayer* OtterLocalPlayer = CastChecked<UOtterLocalPlayer>(LocalPlayer);
		OtterLocalPlayer->GetLocalSettings()->ApplyScalabilitySettings();
	}
...
UGameSettingCollection* UOtterGameSettingRegistry::InitializeVideoSettings(UOtterLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			UOtterSettingValueDiscrete_Resolution* Setting = NewObject<UOtterSettingValueDiscrete_Resolution>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UOtterSettingsShared>()->GetColorBlindMode());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UOtterSettingsShared>()->GetColorBlindStrength());
		...
		//----------------------------------------------------------------------------------
		{
			UOtterSettingAction_SafeZoneEditor* Setting = NewObject<UOtterSettingAction_SafeZoneEditor>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const FOtterQualityDeviceProfileVariant& Variant : PlatformSettings->UserFacingDeviceProfileOptions)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			UOtterSettingValueDiscrete_MobileFPSType* Setting = NewObject<UOtterSettingValueDiscrete_MobileFPSType>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EOtterFramePacingMode::MobileStyle));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetCustomAction([](ULocalPlayer* LocalPlayer)
			{
				...
				{
					UOtterSettingsLocal::Get()->ResetToMobileDeviceDefaults();
				}
				else
				{
					const UOtterLocalPlayer* OtterLocalPlayer = CastChecked<UOtterLocalPlayer>(LocalPlayer);
					...
					OtterLocalPlayer->GetLocalSettings()->RunAutoBenchmark(bImmediatelySaveState);
				}
			});
			...
			{
				const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
				const bool bCanUseDueToMobile = (PlatformSettings->FramePacingMode == EOtterFramePacingMode::MobileStyle);

				const UOtterLocalPlayer* OtterLocalPlayer = CastChecked<UOtterLocalPlayer>(LocalPlayer);
				const bool bCanBenchmark = OtterLocalPlayer->GetLocalSettings()->CanRunAutoBenchmark();
		...
		//----------------------------------------------------------------------------------
		{
			UOtterSettingValueDiscrete_OverallQuality* Setting = NewObject<UOtterSettingValueDiscrete_OverallQuality>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EOtterFramePacingMode::ConsoleStyle, EFramePacingEditCondition::DisableIf));
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EOtterFramePacingMode::DesktopStyle));
...
void AddFrameRateOptions(UGameSettingValueDiscreteDynamic_Number* Setting)
{
	...
	for (int32 Rate : GetDefault<UOtterPerformanceSettings>()->DesktopFrameRateLimits)
	{
...
void UOtterGameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, UOtterLocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		UGameSettingValueDiscreteDynamic_Number* Setting = NewObject<UGameSettingValueDiscreteDynamic_Number>();
		...
		Setting->SetDefaultValue(GetDefault<UOtterSettingsLocal>()->GetFrameRateLimit_OnBattery());
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UOtterSettingsLocal>()->GetFrameRateLimit_InMenu());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EOtterFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UOtterSettingsLocal>()->GetFrameRateLimit_WhenBackgrounded());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EOtterFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UOtterSettingsLocal>()->GetFrameRateLimit_Always());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EOtterFramePacingMode::DesktopStyle));
