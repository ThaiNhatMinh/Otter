...
#include "OtterSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EOtterAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UOtterSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UOtterSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UOtterSettingsShared, FOnSettingChangedEvent, UOtterSettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	UOtterSettingsShared();
	...
	static UOtterSettingsShared* CreateTemporarySettings(const UOtterLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UOtterSettingsShared* LoadOrCreateSettings(const UOtterLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UOtterSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UOtterLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EOtterAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EOtterAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EOtterAllowBackgroundAudioSetting AllowAudioInBackground = EOtterAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EOtterGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EOtterGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EOtterGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EOtterGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EOtterGamepadSensitivity GamepadLookSensitivityPreset = EOtterGamepadSensitivity::Normal;
	UPROPERTY()
	EOtterGamepadSensitivity GamepadTargetingSensitivityPreset = EOtterGamepadSensitivity::Normal;
