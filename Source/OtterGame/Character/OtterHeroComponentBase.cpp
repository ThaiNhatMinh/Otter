...
#include "OtterHeroComponentBase.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterHeroComponentBase)
...
const FName UOtterHeroComponentBase::NAME_BindInputsNow("BindInputsNow");
const FName UOtterHeroComponentBase::NAME_ActorFeatureName("Hero");

UOtterHeroComponentBase::UOtterHeroComponentBase(const FObjectInitializer& ObjectInitializer)
...
void UOtterHeroComponentBase::OnRegister()
{
	Super::OnRegister();

	if (!GetPawn<APawn>())
	{
		UE_LOG(LogOtter, Error, TEXT("[UOtterHeroComponentBase::OnRegister] This component has been added to a blueprint whose base class is not a Pawn. To use this component, it MUST be placed on a Pawn Blueprint."));
...
bool UOtterHeroComponentBase::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	check(Manager);
...
void UOtterHeroComponentBase::HandleChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState)
{
...
void UOtterHeroComponentBase::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
...
void UOtterHeroComponentBase::CheckDefaultInitialization()
{
...
void UOtterHeroComponentBase::BeginPlay()
{
	Super::BeginPlay();
...
void UOtterHeroComponentBase::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UOtterHeroComponentBase::InitializePlayerInput(UInputComponent* PlayerInputComponent)
{
		...
		if (const UOtterPawnData* PawnData = PawnExtComp->GetPawnData<UOtterPawnData>())
		{
			if (const UOtterInputConfig* InputConfig = PawnData->InputConfig)
			{
				...
				if (ensureMsgf(OtterIC, TEXT("Unexpected Input Component class! The Gameplay Abilities will not be bound to their inputs. Change the input component to UOtterInputComponent or a subclass of it.")))
				{
					...
					InitializePlayerInput(InputConfig, OtterIC);
				}
			}
		}
	}
...
void UOtterHeroComponentBase::AddAdditionalInputConfig(const UOtterInputConfig* InputConfig)
{
	TArray<uint32> BindHandles;
...
void UOtterHeroComponentBase::RemoveAdditionalInputConfig(const UOtterInputConfig* InputConfig)
{
	//@TODO: Implement me!
}

bool UOtterHeroComponentBase::IsReadyToBindInputs() const
{
	return bReadyToBindInputs;
}

void UOtterHeroComponentBase::Input_AbilityInputTagPressed(FGameplayTag InputTag)
{
...
void UOtterHeroComponentBase::Input_AbilityInputTagReleased(FGameplayTag InputTag)
{
	const APawn* Pawn = GetPawn<APawn>();
	if (!Pawn)
	{
...
TSubclassOf<UOtterCameraMode> UOtterHeroComponentBase::DetermineCameraMode() const
{
