...
#include "OtterPerformanceSettings.generated.h"
...
USTRUCT()
struct FOtterQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FOtterPerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<EOtterDisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class EOtterFramePacingMode : uint8
{
...
class UOtterPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UOtterPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UOtterPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FOtterQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EOtterFramePacingMode FramePacingMode = EOtterFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by Otter.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EOtterFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Otter Performance Settings"))
class UOtterPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UOtterPerformanceSettings();
...
public:
	...
	TArray<FOtterPerformanceStatGroup> UserFacingPerformanceStats;
};
