		...
		if (const UOtterPawnData* PawnData = PawnExtComp->GetPawnData<UOtterPawnData>())
		{
			if (const UOtterInputConfig* InputConfig = PawnData->InputConfig)
			{
				...
				if (ensureMsgf(OtterIC, TEXT("Unexpected Input Component class! The Gameplay Abilities will not be bound to their inputs. Change the input component to UOtterInputComponent or a subclass of it.")))
				{
					...
					OtterIC->BindNativeAction(InputConfig, OtterGameplayTags::InputTag_Crouch, ETriggerEvent::Triggered, this, &ThisClass::Input_Crouch, /*bLogIfNotFound=*/ false);
					OtterIC->BindNativeAction(InputConfig, OtterGameplayTags::InputTag_AutoRun, ETriggerEvent::Triggered, this, &ThisClass::Input_AutoRun, /*bLogIfNotFound=*/ false);
				}
			}
		}
	}
...
void UOtterHeroComponent::Input_Move(const FInputActionValue& InputActionValue)
{
	...
	if (AOtterPlayerController* OtterController = Cast<AOtterPlayerController>(Controller))
	{
		OtterController->SetIsAutoRunning(false);
	}
...
void UOtterHeroComponent::Input_Crouch(const FInputActionValue& InputActionValue)
{
	if (AOtterCharacter* Character = GetPawn<AOtterCharacter>())
	{
		Character->ToggleCrouch();
	}
}

void UOtterHeroComponent::Input_AutoRun(const FInputActionValue& InputActionValue)
{
	if (APawn* Pawn = GetPawn<APawn>())
	{
		if (AOtterPlayerController* Controller = Cast<AOtterPlayerController>(Pawn->GetController()))
		{
			// Toggle auto running
...
void UOtterHeroComponent::SetAbilityCameraMode(TSubclassOf<UOtterCameraMode> CameraMode, const FGameplayAbilitySpecHandle& OwningSpecHandle)
{
	if (CameraMode)
	{
...
void UOtterHeroComponent::ClearAbilityCameraMode(const FGameplayAbilitySpecHandle& OwningSpecHandle)
{
