...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterSafeZoneEditor)
...
UOtterSafeZoneEditor::UOtterSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UOtterSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UOtterSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UOtterSettingsLocal::Get()->GetSafeZone());
	
	Button_Done->OnClicked().AddUObject(this, &UOtterSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UOtterSafeZoneEditor::HandleBackClicked);
	}
}

bool UOtterSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UOtterSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UOtterSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UOtterSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UOtterSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UOtterSettingsLocal::Get()->GetSafeZone());
}

void UOtterSafeZoneEditor::HandleDoneClicked()
{
