...
#include "CustomSettings/OtterSettingValueDiscrete_Language.h"
...
UGameSettingCollection* UOtterGameSettingRegistry::InitializeGameplaySettings(UOtterLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			UOtterSettingValueDiscrete_Language* Setting = NewObject<UOtterSettingValueDiscrete_Language>();
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UOtterSettingsLocal>()->ShouldAutoRecordReplays());

			Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
			Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(UOtterReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UOtterSettingsLocal>()->GetNumberOfReplaysToKeep());
