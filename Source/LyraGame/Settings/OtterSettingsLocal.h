...
#include "OtterSettingsLocal.generated.h"
...
USTRUCT()
struct FOtterScalabilitySnapshot
{
	GENERATED_BODY()

	FOtterScalabilitySnapshot();
 ...
 */
UCLASS()
class UOtterSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	static UOtterSettingsLocal* Get();
...
public:
	...
	EOtterStatDisplayMode GetPerfStatDisplayState(EOtterDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EOtterDisplayablePerformanceStat Stat, EOtterStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UOtterSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EOtterDisplayablePerformanceStat, EOtterStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FOtterScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FOtterScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UOtterSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
