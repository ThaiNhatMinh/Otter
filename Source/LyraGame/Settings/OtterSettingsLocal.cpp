...
#include "Audio/OtterAudioSettings.h"
#include "Audio/OtterAudioMixEffectsSubsystem.h"
#include "EnhancedActionKeyMapping.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Otter.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Otter.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Otter.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Otter.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Otter.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Otter.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Otter.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Otter.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Otter.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Otter.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FOtterScalabilitySnapshot::FOtterScalabilitySnapshot()
{
...
namespace OtterSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FOtterScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
		...
		Mode = FOtterScalabilitySnapshot();
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UOtterSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UOtterSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UOtterSettingsLocal::UOtterSettingsLocal()
{
...
void UOtterSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UOtterSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EOtterFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	OtterSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UOtterSettingsLocal::ResetToCurrentSettings()
{
...
void UOtterSettingsLocal::BeginDestroy()
{
...
UOtterSettingsLocal* UOtterSettingsLocal::Get()
{
	return GEngine ? CastChecked<UOtterSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UOtterSettingsLocal::ConfirmVideoMode()
{
...
float UOtterSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EOtterFramePacingMode::DesktopStyle)
	{
...
int32 UOtterSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return OtterSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UOtterSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FOtterScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UOtterSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UOtterSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UOtterSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UOtterSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UOtterSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UOtterSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EOtterStatDisplayMode UOtterSettingsLocal::GetPerfStatDisplayState(EOtterDisplayablePerformanceStat Stat) const
{
	if (const EOtterStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EOtterStatDisplayMode::Hidden;
	}
}

void UOtterSettingsLocal::SetPerfStatDisplayState(EOtterDisplayablePerformanceStat Stat, EOtterStatDisplayMode DisplayMode)
{
	if (DisplayMode == EOtterStatDisplayMode::Hidden)
	{
...
float UOtterSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UOtterSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UOtterSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UOtterSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UOtterSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UOtterSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UOtterSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UOtterSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UOtterSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UOtterSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UOtterSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UOtterSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UOtterSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UOtterSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UOtterSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UOtterSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return OtterSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UOtterSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return OtterSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UOtterSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UOtterSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EOtterFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return OtterSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UOtterSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EOtterFramePacingMode::MobileStyle)
	{
...
void UOtterSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UOtterSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EOtterFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = OtterSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UOtterSettingsLocal::ClampMobileQuality()
{
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EOtterFramePacingMode::MobileStyle)
	{
...
void UOtterSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = OtterSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UOtterSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = OtterSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = OtterSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UOtterSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UOtterSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UOtterSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UOtterSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UOtterSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = OtterSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UOtterSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UOtterSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UOtterAudioMixEffectsSubsystem* OtterAudioMixEffectsSubsystem = World->GetSubsystem<UOtterAudioMixEffectsSubsystem>())
			{
				OtterAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UOtterSettingsLocal::CanRunAutoBenchmark() const
{
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UOtterSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UOtterSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UOtterSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UOtterSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UOtterSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UOtterSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UOtterSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UOtterSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UOtterSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UOtterSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UOtterSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UOtterSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UOtterSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UOtterSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UOtterSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UOtterSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UOtterSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UOtterSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UOtterSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EOtterFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = OtterSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UOtterSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UOtterSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UOtterSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UOtterAudioSettings* OtterAudioSettings = GetDefault<UOtterAudioSettings>())
			{
				...
				if (UObject* ObjPath = OtterAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Otter Audio Settings."));
					}
				}
			}
		}
...
void UOtterSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UOtterSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UOtterPlatformSpecificRenderingSettings* PlatformSettings = UOtterPlatformSpecificRenderingSettings::Get();
	const TArray<FOtterQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FOtterQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UOtterPlatformEmulationSettings* Settings = GetDefault<UOtterPlatformEmulationSettings>();
	...
	{
	case EOtterFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EOtterFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EOtterFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UOtterSettingsLocal::UpdateConsoleFramePacing()
{
...
void UOtterSettingsLocal::UpdateDesktopFramePacing()
{
...
void UOtterSettingsLocal::UpdateMobileFramePacing()
{
...
void UOtterSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
