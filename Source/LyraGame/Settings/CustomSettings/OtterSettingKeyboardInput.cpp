...
#include "OtterSettingKeyboardInput.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterSettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "OtterSettings"

namespace Otter::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("OtterErrors_UnknownMappingName", "Unknown Mapping");
}


UOtterSettingKeyboardInput::UOtterSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UOtterSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return Otter::ErrorMessages::UnknownMappingName;
}

FText UOtterSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UOtterSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UOtterSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UOtterSettingKeyboardInput::GetUserSettings() const
{
	if(UOtterLocalPlayer* OtterLocalPlayer = Cast<UOtterLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = OtterLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void UOtterSettingKeyboardInput::OnInitialized()
{
...
void UOtterSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UOtterSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
FText UOtterSettingKeyboardInput::GetPrimaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::First);
}

FText UOtterSettingKeyboardInput::GetSecondaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::Second);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UOtterSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UOtterSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UOtterSettingKeyboardInput::RestoreToInitial()
{
...
bool UOtterSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UOtterSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool UOtterSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
