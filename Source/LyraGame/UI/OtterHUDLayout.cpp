...
#include "UI/Foundation/OtterControllerDisconnectedScreen.h"
#include "UI/OtterActivatableWidget.h"

#if WITH_EDITOR
#include "CommonUIVisibilitySubsystem.h"
#endif	// WITH_EDITOR

#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterHUDLayout)
...
UOtterHUDLayout::UOtterHUDLayout(const FObjectInitializer& ObjectInitializer)
...
void UOtterHUDLayout::NativeOnInitialized()
{
...
void UOtterHUDLayout::NativeDestruct()
{
...
void UOtterHUDLayout::HandleEscapeAction()
{
...
void UOtterHUDLayout::HandleInputDeviceConnectionChanged(EInputDeviceConnectionState NewConnectionState, FPlatformUserId PlatformUserId, FInputDeviceId InputDeviceId)
{
...
void UOtterHUDLayout::HandleInputDevicePairingChanged(FInputDeviceId InputDeviceId, FPlatformUserId NewUserPlatformId, FPlatformUserId OldUserPlatformId)
{
	const FPlatformUserId OwningLocalPlayerId = GetOwningLocalPlayer()->GetPlatformUserId();
...
bool UOtterHUDLayout::ShouldPlatformDisplayControllerDisconnectScreen() const
{
...
void UOtterHUDLayout::NotifyControllerStateChangeForDisconnectScreen()
{
...
void UOtterHUDLayout::ProcessControllerDevicesHavingChangedForDisconnectScreen()
{
	// We should only ever get here if we have bound to the controller state change delegates
	ensure(ShouldPlatformDisplayControllerDisconnectScreen());
...
void UOtterHUDLayout::DisplayControllerDisconnectedMenu_Implementation()
{
	UE_LOG(LogOtter, Log, TEXT("[%hs] Display controller disconnected menu!"), __func__);
...
void UOtterHUDLayout::HideControllerDisconnectedMenu_Implementation()
{
	UE_LOG(LogOtter, Log, TEXT("[%hs] Hide controller disconnected menu!"), __func__);
