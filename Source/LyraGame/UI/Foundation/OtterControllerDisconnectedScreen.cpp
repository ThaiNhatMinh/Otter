...
#include "OtterControllerDisconnectedScreen.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterControllerDisconnectedScreen)
...
UOtterControllerDisconnectedScreen::UOtterControllerDisconnectedScreen(const FObjectInitializer& ObjectInitializer)
...
void UOtterControllerDisconnectedScreen::NativeOnActivated()
{
	Super::NativeOnActivated();

	if (!HBox_SwitchUser)
	{
		UE_LOG(LogOtter, Error, TEXT("Unable to find HBox_SwitchUser on Widget %s"), *GetNameSafe(this));
		return;
	}

	if (!Button_ChangeUser)
	{
		UE_LOG(LogOtter, Error, TEXT("Unable to find Button_ChangeUser on Widget %s"), *GetNameSafe(this));
		return;
	}
...
bool UOtterControllerDisconnectedScreen::ShouldDisplayChangeUserButton() const
{
...
void UOtterControllerDisconnectedScreen::HandleChangeUserClicked()
{
	ensure(ShouldDisplayChangeUserButton());

	UE_LOG(LogOtter, Log, TEXT("[%hs] Change user requested!"), __func__);
...
void UOtterControllerDisconnectedScreen::HandleChangeUserCompleted(const FPlatformUserSelectionCompleteParams& Params)
{
	UE_LOG(LogOtter, Log, TEXT("[%hs] User change complete!"), __func__);
