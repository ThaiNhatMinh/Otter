...
#include "OtterReticleWidgetBase.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterReticleWidgetBase)

UOtterReticleWidgetBase::UOtterReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UOtterReticleWidgetBase::InitializeFromWeapon(UOtterWeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<UOtterInventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float UOtterReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const UOtterRangedWeaponInstance* RangedWeapon = Cast<const UOtterRangedWeaponInstance>(WeaponInstance))
	{
...
bool UOtterReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const UOtterRangedWeaponInstance* RangedWeapon = Cast<const UOtterRangedWeaponInstance>(WeaponInstance))
	{
...
float UOtterReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{
