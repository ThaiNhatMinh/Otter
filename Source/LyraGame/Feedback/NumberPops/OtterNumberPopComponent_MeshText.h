...
#include "OtterNumberPopComponent_MeshText.generated.h"

class UOtterDamagePopStyle;
...
class UOtterNumberPopComponent_MeshText : public UOtterNumberPopComponent
{
	GENERATED_BODY()

public:

	UOtterNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FOtterNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo, const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FOtterNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FOtterNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<UOtterDamagePopStyle>> Styles;
