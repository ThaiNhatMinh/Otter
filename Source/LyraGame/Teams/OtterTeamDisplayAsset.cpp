...
#include "OtterTeamDisplayAsset.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterTeamDisplayAsset)

void UOtterTeamDisplayAsset::ApplyToMaterial(UMaterialInstanceDynamic* Material)
{
	if (Material)
	{
...
void UOtterTeamDisplayAsset::ApplyToMeshComponent(UMeshComponent* MeshComponent)
{
	if (MeshComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UOtterTeamDisplayAsset::ApplyToNiagaraComponent(UNiagaraComponent* NiagaraComponent)
{
	if (NiagaraComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UOtterTeamDisplayAsset::ApplyToActor(AActor* TargetActor, bool bIncludeChildActors)
{
...
#if WITH_EDITOR
void UOtterTeamDisplayAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	for (UOtterTeamSubsystem* TeamSubsystem : TObjectRange<UOtterTeamSubsystem>())
	{
