...
namespace OtterGameplayTags
{
	...
	// When extending Otter, you can create your own movement modes but you need to update GetCustomMovementModeTagMap()
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(Movement_Mode_Custom, "Movement.Mode.Custom", "This is invalid and should be replaced with custom tags.  See OtterGameplayTags::CustomMovementModeTagMap.");
	...
	FGameplayTag FindTagByString(const FString& TagString, bool bMatchPartialString)
	{
		...
		if (!Tag.IsValid() && bMatchPartialString)
		{
			...
			for (const FGameplayTag& TestTag : AllTags)
			{
				...
				{
					UE_LOG(LogOtter, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
					Tag = TestTag;
					break;
				}
			}
