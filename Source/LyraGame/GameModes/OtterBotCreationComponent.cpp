...
#include "OtterBotCreationComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterBotCreationComponent)

UOtterBotCreationComponent::UOtterBotCreationComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UOtterBotCreationComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	ExperienceComponent->CallOrRegister_OnExperienceLoaded_LowPriority(FOnOtterExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::OnExperienceLoaded));
}

void UOtterBotCreationComponent::OnExperienceLoaded(const UOtterExperienceDefinition* Experience)
{
#if WITH_SERVER_CODE
	if (HasAuthority())
	{
...
void UOtterBotCreationComponent::ServerCreateBots_Implementation()
{
	...
	if (GIsEditor)
	{
		const UOtterDeveloperSettings* DeveloperSettings = GetDefault<UOtterDeveloperSettings>();
...
FString UOtterBotCreationComponent::CreateBotName(int32 PlayerIndex)
{
	FString Result;
...
void UOtterBotCreationComponent::SpawnOneBot()
{
	...
	{
		AOtterGameMode* GameMode = GetGameMode<AOtterGameMode>();
		check(GameMode);
		...
		{
			if (UOtterPawnExtensionComponent* PawnExtComponent = NewController->GetPawn()->FindComponentByClass<UOtterPawnExtensionComponent>())
			{
...
void UOtterBotCreationComponent::RemoveOneBot()
{
	if (SpawnedBotList.Num() > 0)
	{
		...
		if (BotToRemove)
		{
			...
			{
				if (UOtterHealthComponent* HealthComponent = UOtterHealthComponent::FindHealthComponent(ControlledPawn))
				{
...
void UOtterBotCreationComponent::ServerCreateBots_Implementation()
{
	ensureMsgf(0, TEXT("Bot functions do not exist in OtterClient!"));
}
