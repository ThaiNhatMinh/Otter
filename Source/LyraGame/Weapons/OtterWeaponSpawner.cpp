...
#include "OtterWeaponSpawner.h"

#include "AbilitySystemBlueprintLibrary.h"
#include "Components/CapsuleComponent.h"
#include "Components/StaticMeshComponent.h"
#include "Engine/World.h"
#include "Equipment/OtterPickupDefinition.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterWeaponSpawner)
...
// Sets default values
AOtterWeaponSpawner::AOtterWeaponSpawner()
{
	...
	CollisionVolume->OnComponentBeginOverlap.AddDynamic(this, &AOtterWeaponSpawner::OnOverlapBegin);
...
void AOtterWeaponSpawner::BeginPlay()
{
	Super::BeginPlay();
	...
	else if (const UWorld* World = GetWorld())
	{
		...
		{
			UE_LOG(LogOtter, Error, TEXT("'%s' does not have a valid weapon definition! Make sure to set this data on the instance!"), *GetNameSafe(this));	
		}
	}
}

void AOtterWeaponSpawner::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void AOtterWeaponSpawner::Tick(float DeltaTime)
{
...
void AOtterWeaponSpawner::OnConstruction(const FTransform& Transform)
{
...
void AOtterWeaponSpawner::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
...
void AOtterWeaponSpawner::CheckForExistingOverlaps()
{
...
void AOtterWeaponSpawner::AttemptPickUpWeapon_Implementation(APawn* Pawn)
{
	if (GetLocalRole() == ROLE_Authority && bIsWeaponAvailable && UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Pawn))
	{
		TSubclassOf<UOtterInventoryItemDefinition> WeaponItemDefinition = WeaponDefinition ? WeaponDefinition->InventoryItemDefinition : nullptr;
...
void AOtterWeaponSpawner::StartCoolDown()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(CoolDownTimerHandle, this, &AOtterWeaponSpawner::OnCoolDownTimerComplete, CoolDownTime);
	}
}

void AOtterWeaponSpawner::ResetCoolDown()
{
	UWorld* World = GetWorld();
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		if (World)
		{
			World->GetTimerManager().SetTimer(CheckOverlapsDelayTimerHandle, this, &AOtterWeaponSpawner::CheckForExistingOverlaps, CheckExistingOverlapDelay);
		}
	}
...
void AOtterWeaponSpawner::OnCoolDownTimerComplete()
{
	ResetCoolDown();
}

void AOtterWeaponSpawner::SetWeaponPickupVisibility(bool bShouldBeVisible)
{
	WeaponMesh->SetVisibility(bShouldBeVisible, true);
}

void AOtterWeaponSpawner::PlayPickupEffects_Implementation()
{
...
void AOtterWeaponSpawner::PlayRespawnEffects_Implementation()
{
	if (WeaponDefinition != nullptr)
	{
...
void AOtterWeaponSpawner::OnRep_WeaponAvailability()
{
...
void AOtterWeaponSpawner::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(AOtterWeaponSpawner, bIsWeaponAvailable);
}

int32 AOtterWeaponSpawner::GetDefaultStatFromItemDef(const TSubclassOf<UOtterInventoryItemDefinition> WeaponItemClass, FGameplayTag StatTag)
{
	if (WeaponItemClass != nullptr)
	{
		if (UOtterInventoryItemDefinition* WeaponItemCDO = WeaponItemClass->GetDefaultObject<UOtterInventoryItemDefinition>())
		{
