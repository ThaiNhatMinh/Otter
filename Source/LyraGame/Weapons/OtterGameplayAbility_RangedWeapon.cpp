...
#include "OtterGameplayAbility_RangedWeapon.h"
#include "Weapons/OtterRangedWeaponInstance.h"
...
#include "AbilitySystem/OtterGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterGameplayAbility_RangedWeapon)
...
UOtterGameplayAbility_RangedWeapon::UOtterGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UOtterRangedWeaponInstance* UOtterGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UOtterRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UOtterGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogOtterAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UOtterRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UOtterGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UOtterGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UOtterGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Otter_TraceChannel_Weapon;
}

FHitResult UOtterGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UOtterGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UOtterGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EOtterAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UOtterWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UOtterWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EOtterAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EOtterAbilityTargetingSource::CameraTowardsFocus) || (Source == EOtterAbilityTargetingSource::PawnTowardsFocus) || (Source == EOtterAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EOtterAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EOtterAbilityTargetingSource::WeaponForward) || (Source == EOtterAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EOtterAbilityTargetingSource::PawnTowardsFocus) || (Source == EOtterAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UOtterGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (OtterConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, OtterConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UOtterGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UOtterRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EOtterAbilityTargetingSource::CameraTowardsFocus);
		...
		if (OtterConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, OtterConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UOtterGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UOtterRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (OtterConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, OtterConsoleVariables::DrawBulletHitRadius, FColor::Red, false, OtterConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UOtterGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UOtterGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UOtterGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UOtterWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UOtterWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogOtterAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UOtterGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UOtterWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UOtterWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FOtterGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FOtterGameplayAbilityTargetData_SingleTargetHit();
