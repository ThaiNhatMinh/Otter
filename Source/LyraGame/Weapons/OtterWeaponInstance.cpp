...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterWeaponInstance)

class UAnimInstance;
struct FGameplayTagContainer;

UOtterWeaponInstance::UOtterWeaponInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	if (APawn* Pawn = GetPawn())
	{
		...
		{
			if (UOtterHealthComponent* HealthComponent = UOtterHealthComponent::FindHealthComponent(GetPawn()))
			{
...
void UOtterWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UOtterWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void UOtterWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float UOtterWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
TSubclassOf<UAnimInstance> UOtterWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
{
	const FOtterAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UneuippedAnimSet);
	return SetToQuery.SelectBestLayer(CosmeticTags);
}

const FPlatformUserId UOtterWeaponInstance::GetOwningUserId() const
{
...
void UOtterWeaponInstance::ApplyDeviceProperties()
{
...
void UOtterWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void UOtterWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
