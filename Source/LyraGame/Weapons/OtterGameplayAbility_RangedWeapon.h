...
#include "OtterGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UOtterRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EOtterAbilityTargetingSource : uint8
{
...
/**
 * UOtterGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UOtterGameplayAbility_RangedWeapon : public UOtterGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UOtterGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="Otter|Ability")
	UOtterRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UOtterRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EOtterAbilityTargetingSource Source) const;
