...
#include "OtterDevelopmentStatics.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterDevelopmentStatics)

bool UOtterDevelopmentStatics::ShouldSkipDirectlyToGameplay()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UOtterDeveloperSettings>()->bTestFullGameFlowInPIE;
	}
#endif
	return false;
}

bool UOtterDevelopmentStatics::ShouldLoadCosmeticBackgrounds()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UOtterDeveloperSettings>()->bSkipLoadingCosmeticBackgroundsInPIE;
	}
#endif
	return true;
}

bool UOtterDevelopmentStatics::CanPlayerBotsAttack()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return GetDefault<UOtterDeveloperSettings>()->bAllowPlayerBotsToAttack;
	}
#endif
	return true;
}
...
UWorld* UOtterDevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UOtterDevelopmentStatics::GetAllBlueprints()
{
...
UClass* UOtterDevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UOtterDevelopmentStatics::GetAllBlueprints();
...
UClass* UOtterDevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
