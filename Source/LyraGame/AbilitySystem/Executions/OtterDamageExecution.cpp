...
#include "OtterDamageExecution.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OtterDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UOtterCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UOtterDamageExecution::UOtterDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UOtterDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FOtterGameplayEffectContext* TypedContext = FOtterGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UOtterTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UOtterTeamSubsystem>();
	...
	{
		UE_LOG(LogOtterAbilitySystem, Error, TEXT("Damage Calculation cannot deduce a source location for damage coming from %s; Falling back to WORLD_MAX dist!"), *GetPathNameSafe(Spec.Def));
	}

	// Apply ability source modifiers
	float PhysicalMaterialAttenuation = 1.0f;
	float DistanceAttenuation = 1.0f;
	if (const IOtterAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UOtterHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
