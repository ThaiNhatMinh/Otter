...
#include "GameModes/OtterExperienceManager.h"
...
#include "Private/AssetTypeActions_OtterContextEffectsLibrary.h"
#include "ToolMenu.h"
#include "ToolMenus.h"
...
DEFINE_LOG_CATEGORY(LogOtterEditor);
...
{
	return HasNoPlayWorld() && !GetDefault<UOtterDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<UOtterDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
 ...
 * FOtterEditorModule
 */
class FOtterEditorModule : public FDefaultGameModuleImpl
{
	typedef FOtterEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FOtterEditorModule::ModulesChangedCallback);
		...
		// Register the Context Effects Library asset type actions.
		{
			...
			TSharedRef<FAssetTypeActions_OtterContextEffectsLibrary> AssetAction = MakeShared<FAssetTypeActions_OtterContextEffectsLibrary>();
			OtterContextEffectsLibraryAssetAction = AssetAction;
	...
	{
		UOtterExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<UOtterExperienceManager>();
	...
	virtual void ShutdownModule() override
	{
		// Unregister the Context Effects Library asset type actions.
		{
			...
			TSharedPtr<IAssetTypeActions> AssetAction = OtterContextEffectsLibraryAssetAction.Pin();
...
private:
	TWeakPtr<IAssetTypeActions> OtterContextEffectsLibraryAssetAction;
	FDelegateHandle ToolMenusHandle;
};

IMPLEMENT_MODULE(FOtterEditorModule, OtterEditor);
